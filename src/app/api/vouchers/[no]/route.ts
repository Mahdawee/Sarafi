...
export async function GET(_: NextRequest, { params }: { params: Promise<{ no: string }> }) {
  ...
  const hawala = db.prepare(`SELECT h.*, c.name as customer_name, s.name as safe_name FROM hawala h LEFT JOIN customers c ON c.id = h.customer_id LEFT JOIN safes s ON s.id = h.safe_id WHERE h.voucher_no = ? ORDER BY h.id`).all(no);
  ...
  return NextResponse.json({ voucher_no: no, hawala, receipts, dc, exchanges, expenses, transfers });
}
